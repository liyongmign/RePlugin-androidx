...
public abstract class PluginAppCompatActivity extends AppCompatActivity {
    ...
    public void startActivityFromFragment(@NonNull Fragment fragment, Intent intent, int requestCode) {
        if (requestCode == -1) {
            ...
            try {
                Method m = FragmentActivity.class.getDeclaredMethod("allocateRequestIndex", Fragment.class);
                int index = (int) m.invoke(this, fragment);
                newRequestCode = ((index + 1) << 16) + (requestCode & 0xffff);
            } catch (Exception e) {
                // Do Noting
            }
